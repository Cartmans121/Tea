...
/* harmony import */ var _components_formT__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./components/formT */ "./src/js/components/formT.js");
/* harmony import */ var _components_formT__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_components_formT__WEBPACK_IMPORTED_MODULE_1__);
...
;
(function () {
  let forms = document.querySelectorAll('.more__form');
  if (forms.length === 0) {
    return;
  }
  let serialize = function (form) {
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      let separator = i === 0 ? '' : '&';
      if (name) {
        str += separator + name + '=' + encodeURI(value);
      }
    }
    return str;
  };
  let formSend = function (form) {
    ...
    let url = './mail.php';
    xhr.open('POST', url);
    xhr.setRequestHeader('Content-Tyope', 'application/x-www-form-urlencoded');
    xhr.onload = function () {
      if (xhr.response === 'success') {
        document.querySelector('.popup-send').classList.add('is-active');
      } else {
...
const observer = new MutationObserver(function (mutationsList, observer) {
  for (let mutation of mutationsList) {
    if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
      const isActive = popupError.classList.contains('is-active');
      body.classList.toggle('disable-scroll', isActive);
    }
  }
});
observer.observe(popupError, {
  attributes: true
});
...
document.addEventListener('click', function (evt) {
  const isClick = evt.target.closest('.popup-content');
  if (isClick) {
    return;
  }
